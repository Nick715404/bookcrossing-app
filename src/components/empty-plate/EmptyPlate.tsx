...
import { generateWrapForString } from '../../utilities/empty-plate.utils';
...
export default function EmptyPlate({
	...
	return (
		<Div className='empty-plate-box'>
			<>{icon}</>
			<Header mode='primary'>{newTitle}</Header>
			<Text style={{ fontSize: '16px', maxWidth: '340px' }}>{text}</Text>
			<Button onClick={handleClick} style={{ margin: '26px 0px' }} size='m'>
				{label}
			</Button>
		</Div>
	);
