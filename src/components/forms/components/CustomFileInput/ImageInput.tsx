...
export function ImageInput({ go, bookId, edit }: Props) {
	...
	useEffect(() => {
		if (go && !fileError) {
			handleImageUpload(selectedImage, bookId).catch(() => setFileError(true));
		}
	...
	return (
		<FormItem>
			...
			{selectedImage.length > 0 && (
				<CustomImage callback={handleRemoveImage} images={selectedImage} />
			)}
			{!fetchedImage && (
				<input
					ref={inputRef}
